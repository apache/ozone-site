...
For example, to verify the signature of `ozone-2.1.1-src.tar.gz`:

```bash
gpg --verify ozone-2.1.1-src.tar.gz.asc ozone-2.1.1-src.tar.gz
```
...
For example, to verify the checksum of `ozone-2.1.1-src.tar.gz`:

```bash
sha512sum -c ozone-2.1.1-src.tar.gz.sha512
```
