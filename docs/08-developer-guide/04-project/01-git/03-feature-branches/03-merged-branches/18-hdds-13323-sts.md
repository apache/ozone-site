...
**Revoke a single session token** — only the token creator or an S3/tenant admin may revoke.  This adds the token to an OM RocksDB revoked-token table. Subsequent S3 requests using that session token are denied immediately, even before the token's natural expiration.
