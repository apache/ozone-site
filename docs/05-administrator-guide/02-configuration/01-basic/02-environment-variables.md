...
When running Ozone components as `systemd` services, you can define environment variables directly in the service's unit file.

#### Method 1: Using the `Environment` Directive

For a small number of variables, use the `Environment` directive within the `[Service]` block. You can specify it multiple times for different variables.
...
For managing a larger number of variables, it is cleaner to define them in a separate file and source it using the `EnvironmentFile` directive. The file should contain simple `KEY=VALUE` pairs.

**Example `/etc/ozone/datanode.env` file:**
```
OZONE_DATANODE_OPTS="-Djava.net.preferIPv4Stack=false"
OZONE_HEAPSIZE_MAX="8G"
```

**Updated `datanode.service` file:**
```ini
[Service]
EnvironmentFile=/etc/ozone/datanode.env
ExecStart=/opt/ozone/bin/ozone datanode
...
```
Note the path to the environment file; you can place it wherever is appropriate for your system configuration.

After modifying a service unit file or its environment file, you must reload the `systemd` configuration and restart the service for the changes to take effect:
```bash
