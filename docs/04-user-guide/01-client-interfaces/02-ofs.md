...
`ofs` enables accessing Ozone data using the `ofs://` URI scheme. It's particularly designed to work seamlessly with [File System Optimized (FSO) buckets](../../03-core-concepts/01-namespace/02-buckets/04-layouts/02-file-system-optimized.md), which maintain an internal hierarchical directory structure.
