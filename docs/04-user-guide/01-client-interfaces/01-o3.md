...
Ozone shell is the primary interface to interact with Ozone from the command line. Behind the scenes it uses the [Java API](./06-java-client-api.md).
...
[Transparent Data Encryption](../../05-administrator-guide/02-configuration/03-security/05-encryption/02-transparent-data-encryption.md) can be enabled at the bucket level.
...
[//]: # (If the key is in an [FSO]&#40;../../07-system-internals/07-features/01-filesystem-optimization.md&#41; bucket it will be moved to the trash when deleted. Below is the trash location:)
