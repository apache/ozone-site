...
Ozone shell is the primary interface to interact with Ozone from the command line. Behind the scenes it uses the [Java API](./java-client-api).
...
[Transparent Data Encryption](../../administrator-guide/configuration/security/encryption/transparent-data-encryption) can be enabled at the bucket level.
